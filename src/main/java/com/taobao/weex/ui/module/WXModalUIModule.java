...
//        toast.setGravity(Gravity.BOTTOM, 0, 0);
    ...
    int dp2px(Context ctx, float dp) {
        ...
        if (mWXSDKInstance.getContext() instanceof Activity) {
            ...
            if (!TextUtils.isEmpty(param)) {
                try {
                    ...
                    maxLength = jsObj.getInteger(MAX_LENGTH);
                } catch (Exception e) {
                    WXLogUtils.e("[WXModalUIModule] confirm param parse error ", e);
                }
            }
            ...
            FrameLayout viewContainer = new FrameLayout(ctx);
            viewContainer.setPadding(dp2px(ctx, 24), 0, dp2px(ctx, 24), 0);
